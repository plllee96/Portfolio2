...
void FontManager::drawText(HDC hdc, int destX, int destY, char* fontName, int fontSize, int fontWidth, LPCWSTR printString, int length, COLORREF color) {
	...
	HFONT hFont = CreateFont(fontSize, 0, 0, 5, fontWidth, 0, 0, 0,
...
void FontManager::drawText(HDC hdc, int destX, int destY, char * fontName, int fontSize, int fontWidth, LPCWSTR* printStringArray, int length, COLORREF color) {
	int arraySize = sizeof(printStringArray) / sizeof(*printStringArray);
}
